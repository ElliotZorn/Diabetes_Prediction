...
The model was compiled using the Adam optimizer (Adaptive Moment Estimation) and the binary cross-entropy loss function, both well-suited for binary classification tasks. Classification accuracy was used as the primary performance metric.

Training was performed for 100 epochs using a mini-batch size of 16. The dataset was split into 80\% training and 20\% testing data, with the test set serving as the validation set during training. All training was conducted in a standard CPU-based environment.

The mini-batch size of 16 provided a balance between frequent weight updates and computational efficiency, particularly given the small dataset. The training duration was capped at 100 epochs to mitigate overfitting, this was also due to the small dataset.
